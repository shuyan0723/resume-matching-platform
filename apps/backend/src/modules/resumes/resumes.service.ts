...
export class ResumesService {
  ...
  async upload(
    ...
    // 将解析任务加入异步队列（Redis 不可用时降级为同步解析，不阻塞上传响应）
    try {
      ...
      // Redis 不可用，降级为后台异步解析（不阻塞 HTTP 响应）
      console.warn('[ResumesService] Bull 队列不可用（Redis 未连接），降级为同步后台解析');
      setImmediate(async () => {
        try {
          await this.handleResumeParse({
            data: { resumeId: savedResume.id, filePath: savedResume.filePath },
          } as Job<{ resumeId: number; filePath: string }>);
        } catch (parseErr) {
          console.error('[ResumesService] 后台解析失败:', parseErr);
          await this.resumesRepository.update(savedResume.id, {
