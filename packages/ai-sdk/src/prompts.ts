// =============================================================
// AI Prompt 模板集中管理
// 所有模板要求模型输出 JSON，调用方直接 JSON.parse，不匹配时 fallback
// =============================================================
import type { ParsedResumeAI, ParsedJobAI, ResumeSuggestionsAI, MatchReasonAI } from '@app/shared-types';
...
未提到的字段填空字符串/空数组，不要编造。`,
  ...
  } satisfies { system: string; buildUser: (rawText: string, fileType?: string) => string },
...

{
  ...
  } satisfies { system: string; buildUser: (jdText: string) => string },
...

{
  ...
  "suggestions": [
    ...
    buildUser: (resumeData: string, jdText: string) =>
      `【简历 JSON】\n${resumeData}\n\n【目标 JD】\n${jdText.slice(0, 8000)}`,
  } satisfies { system: string; buildUser: (resumeData: string, jdText: string) => string },
...

{
  ...
  } satisfies { system: string; buildUser: (resumeData: string, jobData: string, matchScore: number) => string },
};
...
// 类型别名，便于调用方引用
export type { ParsedResumeAI, ParsedJobAI, ResumeSuggestionsAI, MatchReasonAI };
